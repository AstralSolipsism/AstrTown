...
|------|---------|--------|--------|----------|---------|
| 1 | gateway | 20 | ~2,500 | 45,761 | WebSocket 网关服务 |
| 2 | convex/engine/util | 21 | ~1,800 | 41,155 | 游戏引擎抽象层和工具库 |
| 3 | convex/agent | 4 | ~600 | 23,463 | Agent 对话与记忆系统 |
| 4 | aiTown/基础 | 10 | ~1,200 | 27,123 | 游戏世界基础类 |
| 5 | aiTown/agent | 7 | ~585 | 31,157 | Agent 状态机与行为调度 |
| 6 | convex/根目录 | 11 | ~1,600 | 32,739 | Convex 后端入口/胶水层 |
| 7 | convex/botApi-npcService | 2 | 1,543 | 50,192 | Bot API 和 NPC 服务 |
| 8 | src/components | 26 | ~2,000 | 111,48 | 前端 React UI 组件 |
| 9 | src/其他 | 10 | ~800 | 26,382 | 前端应用入口、Hooks、国际化 |
| 10 | data | 21 | ~1,500 | 26,382 | 静态数据层 |
| 11 | plugin | 10 | 1,480 | 53,928 | AstrBot 插件 |
| **合计** | **11** | **~14,608** | **~470,815** | - |
...
| [`gateway/src/commandRouter.ts`](gateway/src/commandRouter.ts) | ~200 | 7,229 | 命令路由器 |
...
| [`gateway/src/httpRoutes.ts`](gateway/src/httpRoutes.ts) | ~150 | 5,698 | HTTP 路由 |
| [`gateway/src/routes.ts`](gateway/src/routes.ts) | ~140 | 5,079 | 路由定义 |
| [`gateway/src/commandMapper.ts`](gateway/src/commandMapper.ts) | ~150 | 5,053 | 命令映射 |
| [`gateway/src/astrtownClient.ts`](gateway/src/astrtownClient.ts) | ~200 | 7,189 | AstrTown 客户端 |
| [`gateway/src/auth.ts`](gateway/src/auth.ts) | ~120 | 3,896 | 认证模块 |
| [`gateway/src/connectionManager.ts`](gateway/src/connectionManager.ts) | ~50 | 1,252 | 连接管理 |
...
| [`gateway/src/queueRegistry.ts`](gateway/src/queueRegistry.ts) | ~70 | 2,027 | 队列注册表 |
| [`gateway/src/types.ts`](gateway/src/types.ts) | ~200 | 6,711 | 类型定义 |
...
#### 2.2.3 convex/agent 模块 (4个文件)

| 文件路径 | 行数 | 字符数 | 功能描述 |
|---------|------|--------|---------|
| [`AstrTown/convex/agent/conversation.ts`](AstrTown/convex/agent/conversation.ts) | ~400 | 11,914 | 对话生成 |
| [`AstrTown/convex/agent/memory.ts`](AstrTown/convex/agent/memory.ts) | ~200 | 7,000 | 记忆系统 |
| [`AstrTown/convex/agent/embedding.ts`](AstrTown/convex/agent/embedding.ts) | ~100 | 3,500 | Embedding 缓存 |
| [`AstrTown/convex/agent/schema.ts`](AstrTown/convex/agent/schema.ts) | ~50 | 1,000 | Schema 定义 |
...
| [`AstrTown/convex/http.ts`](AstrTown/convex/http.ts) | ~138 | ~2,500 | HTTP 路由 |
...
| 文件路径 | 行数 | 字符数 | 功能描述 |
|---------|------|--------|---------|
| [`AstrTown/convex/botApi.ts`](AstrTown/convex/botApi.ts) | 1,038 | 34,429 | Bot API 服务 |
...
| [`AstrTown/src/components/PlayerDetails.tsx`](AstrTown/src/components/PlayerDetails.tsx) | 305 | 11,148 | 玩家详情 |
...
| 文件路径 | 行数 | 字符数 | 功能描述 |
|---------|------|--------|---------|
| [`astrbot_plugin_astrtown/main.py`](astrbot_plugin_astrtown/main.py) | 301 | 11,234 | 插件主入口 |
| [`astrbot_plugin_astrtown/adapter/astrtown_adapter.py`](astrbot_plugin_astrtown/adapter/astrtown_adapter.py) | 851 | 33,570 | 平台适配器 |
| [`astrbot_plugin_astrtown/adapter/protocol.py`](astrbot_plugin_astrtown/adapter/protocol.py) | 120 | 2,397 | 协议定义 |
...
| 命令路由 | [`commandRouter.ts`](gateway/src/commandRouter.ts) | 将命令分发到对应处理器 |
...
| HTTP 路由 | [`httpRoutes.ts`](gateway/src/httpRoutes.ts) | HTTP API 端点 |
| AstrTown 客户端 | [`astrtownClient.ts`](gateway/src/astrtownClient.ts) | 与 Convex 后端通信 |
| 认证 | [`auth.ts`](gateway/src/auth.ts) | Token 验证和会话管理 |
| 连接管理 | [`connectionManager.ts`](gateway/src/connectionManager.ts) | 连接生命周期管理 |
...
- 功能：根据命令类型映射到对应的处理器
- 实现：使用映射表 `commandMappings` 将命令类型路由到处理函数

**2. 事件分发算法**
- 位置：[`eventDispatcher.ts`](gateway/src/eventDispatcher.ts)
- 功能：将事件推送到订阅了对应事件类型的客户端
- 实现：遍历订阅者，根据事件类型过滤并推送
...
- **依赖**：
  - [`astrtownClient.ts`](gateway/src/astrtownClient.ts) → Convex 后端
...
| 功能 | 实现文件 | 说明 |
|------|---------|------|
| 对话生成 | [`conversation.ts`](AstrTown/convex/agent/conversation.ts) | 使用 LLM 生成对话 |
| 记忆系统 | [`memory.ts`](AstrTown/convex/agent/memory.ts) | 记忆写入、检索、排序 |
| Embedding 缓存 | [`embedding.ts`](AstrTown/convex/agent/embedding.ts) | 向量嵌入缓存 |
| Schema 定义 | [`schema.ts`](AstrTown/convex/agent/schema.ts) | 数据模型定义 |

#### 3.3.3 关键算法

**1. 对话生成算法**
- 位置：[`conversation.ts`](AstrTown/convex/agent/conversation.ts)
- 功能：使用 LLM 生成对话响应
- 实现：
  - 构建对话上下文（系统提示 + 历史消息）
  - 调用 LLM API
...
**2. 记忆检索算法**
- 位置：[`memory.ts`](AstrTown/convex/agent/memory.ts)
- 功能：基于相关性检索记忆
- 实现：
  - 计算查询向量
  - 计算相似度
...
| Agent 操作 | [`agentOperations.ts`](AstrTown/convex/aiTown/agentOperations.ts) | Agent 操作逻辑 |
...
| HTTP 路由 | [`http.ts`](AstrTown/convex/http.ts) | 鉴权、bot API、NPC 服务、Replicate webhook |
...
**2. NPC 历史对话分组**
- 位置：[`npcHistory.ts`](AstrTown/convex/npcHistory.ts)
- 功能：按时间段分组 NPC 对话
- 实现：
  - 时间分桶（今天/昨天/本周/更早）
  - 分组排序
...
**3. 密码哈希**
- 位置：[`auth.ts`](AstrTown/convex/auth.ts)
- 功能：PBKDF2 密码哈希
- 实现：
  - PBKDF2(SHA-256)
  - pepper 加盐
...
该模块包含两个核心文件：
- botApi.ts：外部 Bot API 服务（34,429 字符，1,038 行）
...
- **依赖**：
  ...
  - convex/根目录（auth、world）
  - data/characters
- **被依赖**：
  - gateway（通过 HTTP）
  - AstrBot 插件（通过 WebSocket）
...
| 玩家详情 | [`PlayerDetails.tsx`](AstrTown/src/components/PlayerDetails.tsx) | 右侧面板、对话控制、聊天内容 |
...
| 功能 | 实现文件 | 说明 |
|------|---------|------|
| 插件主入口 | [`main.py`](astrbot_plugin_astrtown/main.py) | 定义 LLM 工具和上下文裁剪 |
| 平台适配器 | [`adapter/astrtown_adapter.py`](astrbot_plugin_astrtown/adapter/astrtown_adapter.py) | WebSocket 连接、消息处理、事件投递 |
...
**3. 上下文裁剪算法**
- 位置：[`main.py`](astrbot_plugin_astrtown/main.py)
- 功能：在 LLM 请求前裁剪上下文
- 实现：
  - 读取 astrtown_max_context_rounds 配置
  - 分离 system 消息和非 system 消息
  - 保留最近的 N 条非 system 消息
...
部分文件过大，维护困难：
- [`botApi.ts`](AstrTown/convex/botApi.ts)：34,429 字符，1,038 行
- [`npcService.ts`](AstrTown/convex/npcService.ts)：15,763 字符，505 行
- [`llm.ts`](AstrTown/convex/util/llm.ts)：25,514 字符

#### 5.2.2 模块职责不清

部分模块职责不够清晰：
- convex/根目录：11 个文件，功能混杂
...
- [`llm.ts`](AstrTown/convex/util/llm.ts)：拆分为多个功能文件
...
| 序号 | 技术债务 | 模块 | 优先级 | 说明 |
|------|---------|------|--------|------|
| 1 | 文件过大 | convex/botApi-npcService | 高 | botApi.ts 1,038 行，npcService.ts 505 行 |
| 2 | 模块职责不清 | convex/根目录 | 高 | 11 个文件功能混杂 |
...
**问题描述**：
- [`convex/根目录`](AstrTown/convex/)：11 个文件，功能混杂
...
| 模块 | 文件数 | 总行数 | 总字符数 |
|------|--------|--------|----------|
| gateway | 20 | ~2,500 | 45,761 |
| convex/engine/util | 21 | ~1,800 | 41,155 |
| convex/agent | 4 | ~600 | 23,463 |
| aiTown/基础 | 10 | ~1,200 | 27,123 |
| aiTown/agent | 7 | ~585 | 31,157 |
| convex/根目录 | 11 | ~1,600 | 32,739 |
| convex/botApi-npcService | 2 | 1,543 | 50,192 |
| src/components | 26 | ~2,000 | 111,48 |
| src/其他 | 10 | ~800 | 26,382 |
| data | 21 | ~1,500 | 26,382 |
| plugin | 10 | 1,480 | 53,928 |
| **合计** | **142** | **~14,608** | **~470,815** |
...
| astrbot_plugin_astrtown/ | 10 | 53,928 |
| 其他 | ~10 | ~5,000 |
...
| 对话生成 | [`conversation.ts`](AstrTown/convex/agent/conversation.ts) | agent |
...
| NPC 历史对话分组 | [`npcHistory.ts`](AstrTown/convex/npcHistory.ts) | convex/根目录 |
| 密码哈希 | [`auth.ts`](AstrTown/convex/auth.ts) | convex/根目录 |
...
| 事件格式化 | [`astrtown_adapter.py`](astrbot_plugin_astrtown/adapter/astrtown_adapter.py) | plugin |
| 上下文裁剪 | [`main.py`](astrbot_plugin_astrtown/main.py) | plugin |
...
| POST | `/api/bot/event/ack` | [`postEventAck`](AstrTown/convex/botApi.ts) | botApi |
| GET | `/api/bot/world` | [`getWorldState`](AstrTown/convex/botApi.ts) | botApi |
| GET | `/api/bot/agent/status` | [`getAgentStatus`](AstrTown/convex/botApi.ts) | botApi |
| POST | `/api/bot/control` | [`postControl`](AstrTown/convex/botApi.ts) | botApi |
| POST | `/api/bot/token/validate` | [`postTokenValidate`](AstrTown/convex/botApi.ts) | botApi |
| POST | `/api/bot/description` | [`postDescriptionUpdate`](AstrTown/convex/botApi.ts) | botApi |

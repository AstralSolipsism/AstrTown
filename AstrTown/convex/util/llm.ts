...
// Schema-time constant for vectorIndex dimensions.
// Convex sandbox prohibits process.env during schema evaluation,
// so this MUST be a compile-time literal.
export const EMBEDDING_DIMENSION: number = OLLAMA_EMBEDDING_DIMENSION;

/** Read LLM_EMBEDDING_DIMENSION from env at runtime (lazy, never at module top-level). */
function getEnvEmbeddingDimension(): number | undefined {
  const raw = process.env.LLM_EMBEDDING_DIMENSION;
  if (raw === undefined) return undefined;
  const trimmed = raw.trim();
  if (/^[1-9]\d*$/.test(trimmed)) return Number(trimmed);
  console.warn(
    `LLM_EMBEDDING_DIMENSION must be a positive integer, got '${raw}'. Ignoring.`,
  );
  return undefined;
}

export function detectMismatchedLLMProvider() {
  const envDim = getEnvEmbeddingDimension();
  if (envDim !== undefined && envDim !== EMBEDDING_DIMENSION) {
    console.warn(
      `LLM_EMBEDDING_DIMENSION env var (${envDim}) differs from schema constant (${EMBEDDING_DIMENSION}). ` +
        `To change it, edit EMBEDDING_DIMENSION in util/llm.ts and redeploy.`,
    );
  }
  ...
  switch (EMBEDDING_DIMENSION) {
    ...
    case OLLAMA_EMBEDDING_DIMENSION:
      break;
    default:
      // Any other embedding dimension is treated as custom.
...
export function getLLMConfig(): LLMConfig {
  ...
  if (provider ? provider === 'openai' : process.env.OPENAI_API_KEY) {
    if (EMBEDDING_DIMENSION !== OPENAI_EMBEDDING_DIMENSION) {
      console.warn(
        `EMBEDDING_DIMENSION ${EMBEDDING_DIMENSION} does not match OpenAI recommended dimension ${OPENAI_EMBEDDING_DIMENSION}`,
      );
    }
    return {
      provider: 'openai',
  ...
  if (process.env.TOGETHER_API_KEY) {
    if (EMBEDDING_DIMENSION !== TOGETHER_EMBEDDING_DIMENSION) {
      console.warn(
        `EMBEDDING_DIMENSION ${EMBEDDING_DIMENSION} does not match Together.ai recommended dimension ${TOGETHER_EMBEDDING_DIMENSION}`,
      );
    }
    return {
      provider: 'together',
  ...
  if (EMBEDDING_DIMENSION !== OLLAMA_EMBEDDING_DIMENSION) {
    console.warn(
      `EMBEDDING_DIMENSION ${EMBEDDING_DIMENSION} does not match Ollama recommended dimension ${OLLAMA_EMBEDDING_DIMENSION}`,
    );
  }

  ...
  tick(game: Game, now: number) {
    ...
    if (member1.status.kind === 'walkingOver' && member2.status.kind === 'walkingOver') {
      if (playerDistance < CONVERSATION_DISTANCE) {
        ...
        // 外控相关：对话真正开始（双方 walkingOver -> participating）时推送 conversation.started
        const participantIds = [player1.id, player2.id];
        for (const pId of participantIds) {
          ...
          game.pendingOperations.push({
            ...
            args: {
              ...
              otherParticipantIds: participantIds.filter((x) => x !== pId),
            },
          });
        }
      }

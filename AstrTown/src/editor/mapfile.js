...
function getCurrentTilesetRef() {
    const resource = g_ctx.getActiveTilesetResource?.() || null;
    return {
        kind: 'tileset',
        sourceKind: resource?.sourceKind || 'builtin',
        assetId: resource?.assetId ?? resource?.meta?.assetId ?? null,
        version: resource?.version ?? resource?.meta?.version ?? null,
        path: resource?.path || g_ctx.tilesetpath || '',
    };
}

function buildAnimatedSheetRef(atile) {
    const registryEntry = g_ctx.getActiveSpritesheetResource?.()
        || (typeof g_ctx.getResourceRegistry === 'function'
            ? g_ctx.getResourceRegistry()?.spritesheets?.find((item) => item?.name === atile?.spritesheetname || item?.key === atile?.spritesheetname)
            : null)
        || null;
    const resolvedName = atile?.spritesheetname || registryEntry?.name || registryEntry?.key || '';
    return {
        kind: 'sceneAnimation',
        sourceKind: registryEntry?.sourceKind || 'builtin',
        assetId: registryEntry?.assetId ?? registryEntry?.meta?.assetId ?? null,
        version: registryEntry?.version ?? registryEntry?.meta?.version ?? null,
        name: resolvedName,
        path: registryEntry?.path || resolvedName,
    };
}

function generate_preamble() {
    const tilesetRef = getCurrentTilesetRef();
    const mapfile_preamble = '' +
        '// Map generated by assettool.js [' + new Date() + ']\n' +
        '\n' +
        'export const tilesetRef = ' + JSON.stringify(tilesetRef, null, 2) + '\n' +
        'export const tilesetpath = ' + JSON.stringify(tilesetRef.path) + '\n' +
...
function write_map_file(bg_tiles_0, bg_tiles_1, obj_tiles_1, obj_tiles_2, animated_tiles, object_instances = [], zones = []){
    ...
    for(let x = 0 ; x < animated_tiles.length; x++){
        ...
        const sheetRef = buildAnimatedSheetRef(atile);
        const serializedSprite = {
            ...
            sheet: sheetRef.name,
            sheetRef,
            animation: atile.animationname,
        };
        text += JSON.stringify(serializedSprite, null, 2) + ',\n';
